...
class Processor {
	...
	public function run() {
		...
		if ( ! $batch = Batch::get( $this->args['batch_id'] ) ) {
			return new \WP_Error( 'gravityview/import/errors/invalid_id', __( 'Batch does not exist', 'gravityview-importer' ) );
		}
	...
	private function tick( $batch ) {
		if ( $batch['status'] === $this->args['breakpoint'] ) {
			...
			return new \WP_Error( 'gravityview/import/breakpoint', sprintf( __( 'Breakpoint on status %s', 'gravityview-importer' ), $batch['status'] ), $batch );
		}
	...
	public function handle_parsing( $batch ) {
		...
		if ( $batch['form_id'] ) {
			if ( ! $form = \GFAPI::get_form( $batch['form_id'] ) ) {
				return new \WP_Error( 'gravityview/import/errors/invalid_form', __( 'Invalid form ID.', 'gravityview-importer' ) );
			}
		} else {
			$form = null;
		}
		...
		$interpreter->addObserver( function( $row ) use ( &$batch, &$headers, &$columns, &$all_funcs, &$number, $form, $excerpt_size, $processor ) {
			...
			if ( empty( $headers ) ) {
				...
				if ( empty( $batch['scheme'] ) && $form ) {
					...
					foreach ( $headers as $i => $column ) {
						 ...
						 * The note field.
						 */
						if ( strpos( $column_name, Core::strtolower( __( 'Notes', 'gravityview-importer' ) ) ) !== false ) {
							$columns[ $i ] = array(
								'title' => $column,
								'field' => 'notes',
							);
						 ...
						 * User ID.
						 */
						$user_id_parts = explode( '(', __( 'Created By (User Id)', 'gravityview-importer' ) );
						if (
						 ...
						 */
						if (
							strpos( $column_name, Core::strtolower( __( 'User IP', 'gravityview-importer' ) ) ) !== false
							|| strpos( $column_name, Core::strtolower( __( 'IP Address', 'gravityview-importer' ) ) ) !== false ) {
							$columns[ $i ] = array(
								'title' => $column,
								'field' => 'created_by',
							);
			...
			if ( empty( $batch['scheme'] ) ) {
				foreach ( $row as $i => $data ) {
					...
					if ( empty( $columns[ $i ] ) ) {
						$columns[ $i ] = array(
							'title' => rgar( $headers, $i, sprintf( __( 'Column %d', 'gravityview-importer' ), $i ) ),
							'field' => '',
						);
					}
	...
	public function handle_processing( $batch ) {
		...
		if ( empty( $batch['schema'] ) ) {
			return new \WP_Error( 'gravityview/import/errors/invalid_schema', __( 'No schema provided. Nothing to do.', 'gravityview-importer' ) );
		}
		...
		if ( is_null( $batch['form_id'] ) ) {
			...
			foreach ( $batch['schema'] as &$column ) {
				...
				$input_id = implode( '.', array_filter( array( $field_id, $input ), 'strlen' ) );
					...
					if ( ! $field->enableOtherChoice && ! in_array( 'soft', $batch['flags'] ) ) {
						add_filter( 'gform_field_validation', $validate_callbacks[] = function( $result, $value, $form, $the_field ) use ( &$field, &$batch ) {
							...
							if ( ! in_array( $value, $values ) && apply_filters( 'gravityview/import/column/radio/strict', $strict_radio_choices, $field, $batch ) ) {
								...
								$result['message']  = sprintf( __( 'Radio choice is not one of: %s', 'gravityview-importer' ), esc_html( implode( ', ', $values ) ) );
							}
							return $result;
						}, 10, 4 );
					}
				...
				if ( 'select' === $field->type && ! in_array( 'soft', $batch['flags'] ) ) {
					add_filter( 'gform_field_validation', $validate_callbacks[] = function( $result, $value, $form, $the_field ) use ( &$field, &$batch ) {
						...
						if ( ! in_array( $value, $values ) && apply_filters( 'gravityview/import/column/select/strict', true, $field, $batch ) ) {
							$result['is_valid'] = false;
							$result['message']  = sprintf( __( 'Select choice is not one of: %s', 'gravityview-importer' ), esc_html( implode( ', ', $values ) ) );
						}
						return $result;
					}, 10, 4 );
				}
					...
					add_filter( 'gform_field_validation', $validate_callbacks[] = function( $result, $value, $form, $the_field ) use ( &$field, &$batch ) {
						...
						if ( is_array( $value ) ) {
							if ( array_diff( array_filter( $value ), $values = wp_list_pluck( $field->choices, 'value' ) ) ) {
								$result['is_valid'] = false;
								$result['message']  = sprintf( __( 'Poll/Quiz choice is not one of: %s', 'gravityview-importer' ), esc_html( implode( ', ', $values ) ) );
							}
						} else {
				...
				if ( 'survey' === $field->type && ! in_array( $field->inputType, array( 'text', 'textarea', 'rank' ) ) ) {
					add_filter( 'gform_field_validation', $validate_callbacks[] = function( $result, $value, $form, $the_field ) use ( &$field, &$batch ) {
						...
						if ( is_array( $value ) ) {
							if ( array_diff( array_filter( $value ), $values = wp_list_pluck( $field->choices, 'value' ) ) ) {
								$result['is_valid'] = false;
								$result['message']  = sprintf( __( 'Survey choice is not one of: %s', 'gravityview-importer' ), esc_html( implode( ', ', $values ) ) );
							}
						} else {
							if ( ! in_array( $value, $values = wp_list_pluck( $field->choices, 'value' ) ) ) {
								$result['is_valid'] = false;
					...
					if ( ! in_array( 'soft', $batch['flags'] ) ) {
						add_filter( 'gform_field_validation', $validate_callbacks[] = function( $result, $value, $form, $the_field ) use ( &$field, &$batch ) {
							...
							if ( array_diff( array_filter( $value ), $values ) && apply_filters( 'gravityview/import/column/multiselect/strict', true, $field, $batch ) ) {
								$result['is_valid'] = false;
								$result['message']  = sprintf( __( 'Multiselect choice is not one of: %s', 'gravityview-importer' ), esc_html( implode( ', ', $values ) ) );
							}
							return $result;
						}, 10, 4 );
					}
		...
		foreach ( $batch['schema'] as $column ) {
			...
			if ( Core::is_entry_column( $column['field'] ) ) {
				...
				if ( is_null( $notes ) && 10 < \GFCommon::safe_strlen( $note_string ) ) {
					if ( ! in_array( 'soft', $batch['flags'] ) ) {
						$batch['status'] = 'error';
						$batch['error']  = sprintf( __( 'Entry Notes are not properly formatted.', 'gravityview-importer' ), $row->id );
						$notes           = array();
		...
		if ( ! empty( $update_id ) ) {
			...
			if ( ! $has_user_agent ) {
				 ...
				 */
				$update_entry_properties['user_agent'] = apply_filters( 'gravityview-import/user-agent', __( 'GravityView Import', 'gravityview-importer' ) );
	...
	public function handle_done( $batch ) {
		...
		return new \WP_Error( 'gravityview/import/errors/done', __( 'No more rows to process.', 'gravityview-importer' ) );
	}
	...
	public function handle_rollback( $batch ) {
		...
		if ( in_array( 'nolog', $batch['flags'] ) ) {
			return new \WP_Error( 'gravityview/import/errors/rollback', __( 'Rollback not possible on a non-logged batch.', 'gravityview-importer' ) );
		}

		// @todo test concurrency here
		if ( ! Log::rollback_batch( $batch['id'] ) ) {
			return new \WP_Error( 'gravityview/import/errors/rollback', __( 'Rollback failed.', 'gravityview-importer' ) );
		}
	...
	public function _handle_check_status( $function, $batch ) {
		...
		if ( ! is_array( $batch ) || empty( $batch['status'] ) || $batch['status'] != $status ) {
			return new \WP_Error( 'gravityview/import/errors/invalid_state', sprintf( __( 'Batch is not in "%s" state, wrong handler called.', 'gravityview-importer' ), $status ) );
		}
	}
	...
	public function _handle_check_source( $batch ) {
		if ( ! is_array( $batch ) || empty( $batch['source'] ) || ! is_readable( $batch['source'] ) ) {
			return new \WP_Error( 'gravityview/import/errors/invalid_source', __( 'Source is no longer readable.', 'gravityview-importer' ) );
		}
	}
	...
	public function to_local( $source, $force = false ) {
		...
		if ( preg_match( '#^(?!file)([a-z]+):\/\/#i', $source, $matches ) ) {
			...
			if ( ! in_array( $scheme, array( 'http', 'https' ) ) ) {
				return new \WP_Error( 'gravityview/import/errors/invalid_source', __( 'Source protocol not supported.', 'gravityview-importer' ) );
			}
			...
			if ( is_wp_error( $result = download_url( $source ) ) ) {
				return new \WP_Error( 'gravityview/import/errors/invalid_source', __( 'Source URL could not be downloaded.', 'gravityview-importer' ) );
			}
			return $result;
		}
		...
		if ( ! file_exists( $source ) ) {
			return new \WP_Error( 'gravityview/import/errors/invalid_source', __( 'Source does not exist.', 'gravityview-importer' ) );
		}

		if ( ! is_readable( $source ) || '/tmp/unreadable' === $source ) {
			return new \WP_Error( 'gravityview/import/errors/invalid_source', __( 'Source is not readable.', 'gravityview-importer' ) );
		}
