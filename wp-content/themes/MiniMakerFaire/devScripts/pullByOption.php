...
$findValue = (isset($_GET['find']) ? $_GET['find'] : '');
$updateValue = (isset($_GET['updateTo']) ? $_GET['updateTo'] : '');
if ($option != '') {
// loop thru blogs
    foreach ($results as $blogrow) {
        ...
        $sql = "select * from " . $table . " where option_name = '" . $option . "'";
        ...
        if (($optionValue != '' && strcmp($parmValue, $optionValue) == 0) || $parmValue == '') {
            if (($findValue !== '' && $findValue == $optionValue) || $findValue == '') {
                if ($updateValue != '') {
                    $wpdb->update(
                            $table,
                            array(
                                'option_value' => $updateValue // text                                
                            ),
