...
function add_sidebar_text_middle($form, $entry) {
    ...
    global $wpdb;
    $results = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}gf_form where is_active = 1 and is_trash = 0");
    ...
    <!-- duplicate entry -->
    <div class="meta-box-sortables">
        <div id="dupEntry" class="postbox">
            <button type="button" class="handlediv button-link" aria-expanded="true"><span class="screen-reader-text">Toggle panel: Duplicate/Copy Entry</span><span class="toggle-indicator" aria-hidden="true"></span></button>
            <h2 class="hndle ui-sortable-handle"><span>Duplicate/Copy Entry</span></h2>
            <div class="inside">
    <?php echo display_dupCopy_entry_box($form, $entry, $formList); ?>
            </div>
        </div>
    </div>
    ...
    if (current_user_can('gravityforms_edit_entries')) {
        ...
        <select id="entry_info_status_change">
            ...
            foreach ($field303['choices'] as $choice) {
                $selected = '';
                if ($lead[$field303['id']] == $choice['text'])
                    $selected = ' selected ';
...
function disp_ratings($form, $lead) {
    ...
    $entry_id = $lead['id'];
    $sql = "SELECT user_id, rating, ratingDate FROM `{$wpdb->prefix}gf_lead_rating` where entry_id = " . $entry_id;
    $ratingTotal = 0;
    $ratingNum = 0;
    $ratingResults = '';
...
function disp_flags($form, $lead) {
    ...
    if (is_array($field304['inputs'])) {
        foreach ($field304['inputs'] as $choice) {
            $selected = '';
            if (stripslashes($lead[$choice['id']]) == stripslashes($choice['label']))
                $selected = ' checked ';
...
function display_dupCopy_entry_box($form, $lead, $formList) {
    ...
    foreach ($formList as $choice) {
        $selected = '';
        if ($choice['id'] == $lead['form_id'])
            $selected = ' selected ';
        $output .= '<option ' . $selected . ' value="' . $choice['id'] . '">' . $choice['title'] . '</option>';
    }
    $output .= '</select><br/><br/>';
    $output .= '<input type="button" id="duplicate_entry_id" name="duplicate_entry_id" value="Duplicate Entry" class="button" style="width:auto;padding-bottom:2px;" /><br />';
    $output .= '<br><span class="updMsg" id="duplicate_entry_idMsg"></span>';
