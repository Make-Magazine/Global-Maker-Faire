...
jQuery(document).ready(function () {
    ...
    //duplicate entry     
    jQuery('#duplicate_entry_id').click(function (e) {
        e.preventDefault();
        var processing_icon = '<i class="fa fa-spinner fa-pulse fa-2x fa-fw"></i><span class="sr-only">Loading...</span>';
        jQuery('#duplicate_entry_idMsg').html(processing_icon);
        
        var entry_id = jQuery("input[name=entry_info_entry_id]").val();
        var data = {
            'action': 'duplicate_entry_id',
            'entry_id': entry_id,
            'entry_form_copy': jQuery("select[name=entry_form_copy]").val()
        };                
        ...
        jQuery.post(ajaxurl, data, function (r) {
            ...
            if (r.result === 'updated') {
                ...
                msg = '<i style="color:green" class="fa fa-check"></i>';
            } else {
                //after update - set meta field status to failed
                msg = '<i style="color:red" class="fa fa-times"></i>';
            }
            jQuery('#duplicate_entry_idMsg').html(msg+'<br/>'+r.msg);
        });
    });
    ...
    jQuery('#add_entry_note').click(function (e) {
        ...
        if (jQuery("[name=otherEmail]").val() !== '') {
            emailTo.push(jQuery("[name=otherEmail]").val());
        }
